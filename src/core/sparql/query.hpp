...
public:
    ...
    /**
     * Indicating what is this query's type.
     */
    enum QueryType { SELECT, ASK };
    ...
    void print_sparql_query() {
        static const char *QueryTypeName[2] = { "SELECT", "ASK" };

        logstream(LOG_INFO) << "SPARQLQuery"
                            << "[ Type=" << QueryTypeName[q_type] 
                            << " | QID=" << qid 
                            << " | PQID=" << pqid 
                            << " | MT_TID=" << mt_tid 
                            << " ]" << LOG_endl;
